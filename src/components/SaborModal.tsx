...
import type { Sabor } from '../store/useMenuStore';
...
export default function SaborModal({ isOpen, onClose, sabor }: Props) {
  const { tipos, addSabor, updateSabor } = useMenuStore();
  const [formData, setFormData] = useState({
    tipo_id: '',
    subtipo: '',
    nombre: '',
    es_componente_gelatina: false,
    nivel: '',
    activo: true
  });
  ...
  useEffect(() => {
    if (sabor) {
      setFormData({
        tipo_id: sabor.tipo_id.toString(),
        subtipo: sabor.subtipo || '',
        nombre: sabor.nombre,
        es_componente_gelatina: sabor.es_componente_gelatina,
        nivel: sabor.nivel || '',
        activo: sabor.activo
      });
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.tipo_id || !formData.nombre) return;
    ...
    try {
      const payload = {
        ...
        es_componente_gelatina: formData.es_componente_gelatina,
        nivel: formData.nivel || null,
        activo: formData.activo
      };

      if (sabor) {
        await updateSabor(sabor.id, payload);
      } else {
        await addSabor(payload);
      }
      onClose();
    } catch (error) {
      console.error(error);
      ...
      <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-white/50 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-white/50 backdrop-blur-md sticky top-0 z-10 w-full shrink-0">
          <h2 className="text-xl font-bold text-gray-800 tracking-tight">
            {sabor ? 'Editar Sabor' : 'Nuevo Sabor'}
          </h2>
          <button 
            type="button"
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 w-full">
          <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5 h-full w-full">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-gray-700">Nombre del Sabor *</label>
              <input 
                required
                type="text"
                placeholder="Ej. Fresa, Chocolate, Nuez"
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#59348b] focus:ring-2 focus:ring-[#59348b]/20 transition-all outline-none text-gray-800 shadow-sm"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-gray-700">Nivel (Opcional)</label>
              <select 
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#59348b] focus:ring-2 focus:ring-[#59348b]/20 transition-all outline-none text-gray-800 shadow-sm"
                value={formData.nivel}
                onChange={e => setFormData({...formData, nivel: e.target.value})}
              >
                <option value="">General (Sin nivel)</option>
                <option value="base">Base</option>
                <option value="final">Final (Decoración/Top)</option>
              </select>
              <span className="text-xs text-gray-500 font-medium">Útil para separar sabores de base y rellenos finales.</span>
            </div>
            
            <label className="flex items-center gap-3 cursor-pointer mt-2 w-fit group">
              <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${formData.es_componente_gelatina ? 'bg-[#59348b] border-[#59348b]' : 'bg-white border-gray-300 group-hover:border-[#59348b]'}`}>
                {formData.es_componente_gelatina && <X className="w-3.5 h-3.5 text-white" style={{rotate: '45deg'}} />}
              </div>
              <input 
                type="checkbox" 
                className="hidden"
                checked={formData.es_componente_gelatina}
                onChange={e => setFormData({...formData, es_componente_gelatina: e.target.checked})}
              />
              <span className="text-sm font-bold text-gray-700 select-none">Es componente de Gelatina</span>
            </label>

            <label className="flex items-center gap-3 cursor-pointer mt-0 w-fit group">
            ...
            <footer className="mt-4 flex items-center justify-end gap-3 border-t border-gray-100 pt-5">
              ...
              >
                {isSubmitting ? 'Guardando...' : 'Guardar Sabor'}
              </button>
            </footer>
          </form>
        </div>
